...
void btn_mode_callback(void* b) {
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if(fd_game_state == GAME_PLAY){
			task_post_pure_msg(FD_GAME_BULLET_ID, FD_GAME_BULLET_SHOOT);
			APP_DBG("[btn_down_callback] FD_GAME_BULLET_SHOOT\n");
		}
		else{
...
void btn_up_callback(void* b) {
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if(fd_game_state == GAME_PLAY){
			task_post_pure_msg(FD_GAME_GUN_ID, FD_GAME_GUN_RIGHT);
			APP_DBG("[btn_down_callback] FD_GAME_GUN_RIGHT\n");
		}
		else{
...
void btn_down_callback(void* b) {
	...
	case BUTTON_SW_STATE_RELEASED: {
		...
		if(fd_game_state == GAME_PLAY){
			APP_DBG("[btn_down_callback] FD_GAME_GUN_LEFT\n");
			task_post_pure_msg(FD_GAME_GUN_ID, FD_GAME_GUN_LEFT);
		}
		else{
